...
- **Онбординг за 1 минуту**: выбор текущего и целевого времени подъёма, длительности челленджа (7/14/21 дней).
...
3. В коде уже реализовано:
   ...
   - `Telegram.WebApp.expand()` → сразу разворачивает WebView на всю высоту (если fullscreen ещё не активирован).
   - `Telegram.WebApp.requestFullscreen()` → вызывается после старта челленджа (первое действие пользователя). При отказе интерфейс остаётся в режиме max-height.
